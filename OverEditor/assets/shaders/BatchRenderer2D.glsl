...
void Sample(sampler2D slot)
{
	...
	if (v_TexRepeat == 1)
	{
		coord -= v_TexRegion.xy;
		coord.x = mod(coord.x, v_TexRegion.z);
		coord.y = mod(coord.y, v_TexRegion.w);
		coord += v_TexRegion.xy;
		if (coord.x < v_TexRegion.x + EPSILON1)
			coord.x += EPSILON2;
		if (coord.y < v_TexRegion.y + EPSILON1)
			coord.y += EPSILON2;
	}
