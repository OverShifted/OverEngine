...
#define UPLOAD_UNIFORM(name) Renderer2D::GetShader()->UploadUniformFloat(#name, name);
    ...
    : Layer("LiquidGlass")
{
    ...
    finalFb = FrameBuffer::Create(props);
}
...
void LiquidGlass::OnUpdate(TimeStep deltaTime)
{
    ...
    // Draw the bg scene
    {
        ...
        Renderer2D::DrawQuad({ -2.0, 10.0, 0.0 }, glm::radians(0.0), { 26, 20.61 }, TexturedQuadProps { .Sprite = bg });
        Renderer2D::EndScene();
    }
    fb->Unbind();
...
void LiquidGlass::OnImGuiRender()
{
    ...
    ImGui::DragFloat("Size", &size, 0.01, 0.0, 5.0);
    ...
    // ImGui::Image((void*) fb->GetColorAttachmentRendererID(), ImGui::GetContentRegionAvail());
}
